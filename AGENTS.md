...
- `crates/server/src/main.rs`: bootstraps config, state, logging, uses SeaORM's entity-first schema sync, and mounts the router.
- `crates/server/src/config.rs`: env-driven settings (HOST, PORT, JWT_SECRET, RUST_LOG, DATABASE_URL, DB_MAX_CONNS, DB_MIN_IDLE).
- `crates/server/src/state.rs`: shared `AppState` (JWT keys, SeaORM `DatabaseConnection`).
- `crates/server/src/auth/`: JWT helpers (`jwt.rs`), password hashing (`password.rs`), and role gate layer (`role_layer.rs`) with shared `Claims`/`Role` types.
- `crates/server/src/db/entities/`: SeaORM entities (one module per entity).
- `crates/server/src/db/dao/`: data access objects for queries/transactions.
- `crates/server/src/services/`: business logic that orchestrates DAOs.
- `crates/server/src/routes/`: feature routers — `public.rs`, `auth.rs` (register/login/refresh), `protected.rs` (/me), `admin.rs` (/admin/stats); merged in `routes/mod.rs`.
- `crates/server/src/error.rs`: consistent JSON error responses; `crates/server/src/logging.rs`: tracing setup.
- `crates/server/examples/client.rs`: Reqwest demo hitting the API; uses `BASE_URL`, `USERNAME`, `PASSWORD`.
- `crates/server/views/`: HTML templates rendered with Askama (e.g., index page).
...
## Testing Guidelines
- Add fast, isolated tests under `crates/server/tests/` or in the same module with `#[cfg(test)]`.
...
- Use SeaORM v2.x for all database access; keep entities in `crates/server/src/db/entities` and queries in `crates/server/src/db/dao`.
